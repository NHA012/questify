...
import { getUserById } from '@/services/admin.srv';
import { showCourseProgress } from '@/services/courseLearning.srv';
import Image from 'next/image';
import { lecturesIcon } from '@/assets/icons';

interface ViewedProfileProps {
  user_id: string;
  // username: string;
  // title: string;
  // email: string;
...
const ViewedProfile: React.FC<ViewedProfileProps> = ({
  user_id,
  // username,
  // title,
  // email,
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        setLoading(true);
        const response = await getUserById(user_id as string);
        if (response) {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        ...
        const response = await showCourseProgress(user_id, currentPage, pageSize);
        ...
        <section className="mb-16">
          <div className="flex gap-12 items-start h-[200px]  max-md:flex-col max-md:gap-8 max-md:items-center">
            ...
            <div className="flex flex-col justify-center h-full">
              <h1 className="mb-3 text-3xl font-semibold text-neutral-800">{userData?.userName}</h1>
              <h2 className="mb-3 text-xl">{userData?.gmail}</h2>
              <p className="mb-3 text-base text-gray-500">{userData?.role}</p>
          ...
          <div className={styles.mainContent}>
            ...
            {activeTab === 'edit_profile' && (
              <Setting
                ...
                photoUrl={userData?.imageUrl}
                userId={userData?.id}
                onUserUpdate={userData?.onUserUpdate}
              />
            )} */}
